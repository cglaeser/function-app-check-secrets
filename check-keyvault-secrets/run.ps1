...
try {
    ...
    $summary = @{
        ...
        KeyVaultsWithExpiringSecrets = 0
        ExpiredSecrets = 0
        ExpiringSecrets = 0
    ...
    foreach ($kv in $keyVaults) {
        ...
        try {
            ...
            if ($keyVaultHasExpiringSecrets) {
                $summary.KeyVaultsWithExpiringSecrets++
            }
    ...
    $responseBody = @{
        ...
        Message = if ($results.Count -eq 0) { 
            ...
            "Found $($summary.ExpiredSecrets + $summary.ExpiringSecrets) expiring or expired secrets across $($summary.KeyVaultsWithExpiringSecrets) Key Vaults." 
        }
        Notes = @(
